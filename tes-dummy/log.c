...
int8_t enco_log_open(const char *_file_,sqlite3 **_db_){
...
int8_t enco_log_close(sqlite3 ** _db_){
 ...
 * @param __col 
 */
void enco_log_add_column(sqlite3 *__db__,char*__table_name,char *__col){
	char *cmd = NULL;
...
int simpan_db(struct log_data_t *_data_){
    ...
    const char *namaFileDb = "dataRMC.db";
    sqlite3 *db=NULL;

    int ret = enco_log_open(namaFileDb,&db);
    if(ret != SQLITE_OK){
        return -1;
    }
    ...
    perintah_db = sqlite3_mprintf("INSERT INTO gprmc(datetime,status,lat,lon,speed)"
                                  "VALUES('%s','%c', %lf, %lf, %lf)",_data_->datetime,
                                                                     _data_->status,
                                                                     _data_->lat,
                                                                     _data_->lon,
                                                                     _data_->speed
